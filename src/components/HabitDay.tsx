...
export function HabitDay({ defaultCompleted = 0, amount = 0, date }: HabitDayProps) {
  ...
  return (
    <Popover.Root>
      <Popover.Trigger
        className={clsx("w-10 border-2 rounded-lg cursor-pointer transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-background", {
          'bg-zinc-900 border-zinc-800': completedPercentage === 0,
          'bg-violet-800 border-violet-500': completedPercentage > 0 && completedPercentage < 20,
